...
- Limited font database creation and handling, i.e. currently QPF fonts — see [Font Database](#font-database)
...
> In general, building Qt 4.8.7 on a recent gcc 14.2.0 in Debian trixie on ARM64 was already annoying enough, but
...
The `QWS_DISPLAY` environment variable on the client needs to be set to the same display if a non-default 
display is used. Alternatively, QWS applications typically also provide a `-display` command line 
option.

See also [launch.json](.vscode/launch.json) or [tasks.json](.vscode/tasks.json) for examples.

## Screen Driver

QWSWayland currently only directs QWS clients to use the VNC screen driver for the primary screen.
In practice, any Qt screen driver should work (at least the common ones) — the client only
queries basic information from it and does not depend on it for rendering (there is no actual
VNC connection between QWSWayland and a QWS client).
...
the VNC screen driver can be built into Qt with minimal extra effort and Qt needed to be built anyway,
it is the pragmatic choice for now.

Unfortunately, that's the somewhat annoying thing about screen drivers in QWS: clients want direct hardware
access to the screen rather than delegating everything to the server — comparable to how Qt 6's Wayland
...
unexplained flood of missing attributes in source files generated by `uic4` from `.ui` definition files 
(e.g. basically all dialogs like QFileDialog and so on).
